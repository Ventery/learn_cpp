...
int main(int argc, char *argv[])
{
	...
	setsockopt(sock0, SOL_SOCKET, SO_REUSEADDR, &option, optlen);
	setsockopt(sock0, SOL_SOCKET, SO_REUSEPORT, &option, optlen);
	...
	if (bind(sock0, (struct sockaddr*)&bind_adr, sizeof(bind_adr)) == -1)
		error_handling("bind() error");
	...
	serv_adr.sin_addr.s_addr = inet_addr("140.82.48.113");

	if (connect(sock0, (struct sockaddr*)&serv_adr, sizeof(serv_adr)) == -1)
		error_handling("connect() error!");
	sleep(10000);
